...
mongoose.connect('mongodb+srv://adiiitya:mongodb%40h2r@cluster0.zmyzxwz.mongodb.net/');
...
app.post('/api/foods/insert',async(req,res)=>{
  ...
  app.get('/getfoods/:authorId',async(req,res)=>{
  try { 
 ...
 if (cachedData) {
  ...
  },   
  {
    $group:{
      _id:"$authorId",
      totalfoods:{$sum:1},
      foodTitle:{$first:'$foodTitle'},
      avgfoodLength:{$avg:{$strLenCP:"$foodContent"}}
    },
  },
  {
  $sort:{
  ...
  app.delete('/api/foods/deleteOne', async (req, res) => {
    try {
        if (req.body) {
  ...
  app.put('/api/foods/updateOne/:id', async (req, res) => {
    try {  
        if (req.body) {
